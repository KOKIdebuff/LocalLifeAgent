(function () {
  ...
  function renderRoute() {
    ...
    if (state.route.name === "plan-branches") {
      renderPlanBranchesPage(state.route.planId);
      return;
    }
    if (state.route.name === "plan-branch-detail") {
      renderPlanBranchDetailPage(state.route.planId, state.route.branchId);
      return;
    }
  ...
  function createFeedbackListPanel(data) {
    ...
    createBranch.type = "button";
    createBranch.className = "secondary-btn";
    setButtonContent(createBranch, "difference", "生成待采纳调整");
    createBranch.disabled = !data.feedback.length;
    createBranch.addEventListener("click", function () {
      createBranchFromFeedback(data);
    });
    heading.append(title, createBranch);
  ...
  async function createBranchFromFeedback(data) {
    if (!data || !data.share) return;
    setRouteNotice("正在生成待采纳调整...", "");
    try {
      const response = await fetch("/api/plans/" + encodeURIComponent(data.share.planId) + "/branches", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sourceShareId: data.share.shareId,
          baseVersion: data.share.planVersion,
          feedbackIds: (data.feedback || []).map(function (item) { return item.feedbackId; }),
          idempotencyKey: createClientId("branch"),
          actor: "owner",
        }),
      });
      const result = await response.json();
      if (!response.ok || !result.ok || !result.branch) {
        throw new Error(result && result.error ? result.error : "branch_create_failed");
      }
      navigateTo("/plans/" + encodeURIComponent(data.share.planId) + "/branches/" + encodeURIComponent(result.branch.branchId));
    } catch (error) {
      setRouteNotice("待采纳调整没有生成，请稍后重试。", "warning");
    }
  }

  function renderPlanBranchesPage(planId) {
    els.routeBreadcrumbCurrent.textContent = "待采纳调整";
    els.routeKicker.textContent = "PLAN CHANGES";
    els.routeTitle.textContent = "待采纳调整";
    els.routeSubtitle.textContent = "查看根据协同反馈生成的调整，采纳前不会覆盖当前方案。";
    els.routeActions.innerHTML = "";
    els.routeContent.className = "route-content";
    els.routeContent.innerHTML = "";
    setRouteNotice("正在读取待采纳调整...", "");
    els.routeContent.appendChild(createRouteStatePanel({
      tone: "loading",
      icon: "hourglass_top",
      title: "正在读取调整记录",
      text: "这里会展示当前方案和所有待处理调整。",
    }));
    fetchPlanBranches(planId, null);
  }

  function renderPlanBranchDetailPage(planId, branchId) {
    els.routeBreadcrumbCurrent.textContent = "调整详情";
    els.routeKicker.textContent = "CHANGE DETAIL";
    els.routeTitle.textContent = "调整详情";
    els.routeSubtitle.textContent = "对比当前方案与待采纳调整，再决定采纳或拒绝。";
    els.routeActions.innerHTML = "";
    els.routeContent.className = "route-content";
    els.routeContent.innerHTML = "";
    setRouteNotice("正在读取调整详情...", "");
    els.routeContent.appendChild(createRouteStatePanel({
      tone: "loading",
      icon: "difference",
      title: "正在读取调整详情",
      text: "如果记录有效，你会看到反馈来源、差异摘要和处理入口。",
    }));
    fetchPlanBranches(planId, branchId);
  }

  async function fetchPlanBranches(planId, branchId) {
    try {
      const url = "/api/plans/" + encodeURIComponent(planId) + "/branches" +
        (branchId ? "/" + encodeURIComponent(branchId) : "");
      const response = await fetch(url);
      const data = await response.json();
      if (!response.ok || !data.ok) {
        throw new Error(data && data.error ? data.error : "branches_unavailable");
      }
      state.activePlanBranches = data;
      if (branchId) {
        renderPlanBranchDetailContent(planId, data.branch || findBranch(data, branchId), data);
      } else {
        renderPlanBranchesContent(planId, data);
      }
    } catch (error) {
      setRouteNotice("暂时无法读取调整记录。", "warning");
      showRouteState({
        tone: "warning",
        icon: "error",
        title: "调整记录暂时不可用",
        text: "可以稍后重试；已有方案和分享记录不会被清空。",
        actionLabel: "返回方案详情",
        actionIcon: "arrow_back",
        onAction: function () { navigateTo("/plans/" + encodeURIComponent(planId)); },
      });
    }
  }

  function findBranch(data, branchId) {
    return (data.branches || []).find(function (branch) {
      return branch.branchId === branchId;
    }) || null;
  }

  function renderPlanBranchesContent(planId, data) {
    const proposed = (data.derivedBranches || []).filter(function (branch) {
      return branch.status === "proposed";
    });
    els.routeActions.innerHTML = "";
    const back = document.createElement("button");
    back.type = "button";
    back.className = "ghost-btn";
    setButtonContent(back, "arrow_back", "方案详情");
    back.addEventListener("click", function () { navigateTo("/plans/" + encodeURIComponent(planId)); });
    const rollback = document.createElement("button");
    rollback.type = "button";
    rollback.className = "secondary-btn";
    setButtonContent(rollback, "undo", "回到上一版");
    rollback.disabled = !canRollback(data.mainBranch);
    rollback.addEventListener("click", function () {
      rollbackPreviousMain(planId);
    });
    els.routeActions.append(back, rollback);
    setRouteNotice(
      proposed.length ? proposed.length + " 个调整等待处理。" : "当前没有待采纳调整。",
      proposed.length ? "warning" : "success"
    );
    els.routeContent.className = "route-content branch-page";
    els.routeContent.innerHTML = "";
    els.routeContent.appendChild(createPageSummaryPanel({
      kicker: "CHANGE QUEUE",
      title: proposed.length + " 个待采纳调整",
      text: "调整来自本地协同反馈。采纳后才会生成新的当前版本，拒绝后仍保留处理记录。",
      tags: [
        { text: "显式采纳", tone: "amber" },
        { text: "一步回滚", tone: canRollback(data.mainBranch) ? "blue" : "" },
      ],
      metrics: [
        { label: "当前版本", value: data.mainBranch ? String(data.mainBranch.baseVersion) : "暂无", tone: "blue" },
        { label: "待处理", value: String(proposed.length), tone: proposed.length ? "amber" : "green" },
        { label: "已记录", value: String((data.branches || []).length) },
      ],
    }));
    if (!proposed.length) {
      els.routeContent.appendChild(createRouteStatePanel({
        icon: "check_circle",
        title: "暂无待采纳调整",
        text: "从协同反馈详情页生成调整后，会出现在这里。",
      }));
      return;
    }
    const list = document.createElement("section");
    list.className = "branch-list";
    proposed.forEach(function (branch) {
      list.appendChild(createBranchCard(planId, branch));
    });
    els.routeContent.appendChild(list);
  }

  function renderPlanBranchDetailContent(planId, branch, data) {
    if (!branch) {
      showRouteState({
        tone: "warning",
        icon: "travel_explore",
        title: "没有找到这条调整",
        text: "它可能已经被处理，或当前本地记录不可用。",
        actionLabel: "查看调整列表",
        actionIcon: "list",
        onAction: function () { navigateTo("/plans/" + encodeURIComponent(planId) + "/branches"); },
      });
      return;
    }
    els.routeTitle.textContent = branch.snapshot && branch.snapshot.selectedPlan && branch.snapshot.selectedPlan.name || "待采纳调整";
    els.routeActions.innerHTML = "";
    const back = document.createElement("button");
    back.type = "button";
    back.className = "ghost-btn";
    setButtonContent(back, "arrow_back", "调整列表");
    back.addEventListener("click", function () {
      navigateTo("/plans/" + encodeURIComponent(planId) + "/branches");
    });
    const reject = document.createElement("button");
    reject.type = "button";
    reject.className = "ghost-btn";
    setButtonContent(reject, "block", "暂不处理");
    reject.disabled = branch.status !== "proposed";
    reject.addEventListener("click", function () {
      rejectPlanBranch(planId, branch.branchId);
    });
    const adopt = document.createElement("button");
    adopt.type = "button";
    adopt.className = "primary-btn";
    setButtonContent(adopt, "check_circle", "使用这版方案");
    adopt.disabled = branch.status !== "proposed";
    adopt.addEventListener("click", function () {
      adoptPlanBranch(planId, branch);
    });
    els.routeActions.append(back, reject, adopt);
    setRouteNotice(branchStatusCopy(branch), branch.status === "proposed" ? "warning" : "success");
    els.routeContent.className = "route-content branch-detail-page";
    els.routeContent.innerHTML = "";
    els.routeContent.appendChild(createBranchDetailSummary(branch, data.mainBranch));
    els.routeContent.appendChild(createBranchDiffPanel(branch));
    els.routeContent.appendChild(createBranchComparePanel(data.mainBranch, branch));
  }

  function createBranchCard(planId, branch) {
    const card = document.createElement("article");
    card.className = "branch-card";
    const content = document.createElement("div");
    const kicker = document.createElement("p");
    kicker.className = "section-kicker";
    kicker.textContent = branchStatusLabel(branch.status);
    const title = document.createElement("h2");
    title.textContent = branch.snapshot && branch.snapshot.selectedPlan && branch.snapshot.selectedPlan.name || "待采纳调整";
    const text = document.createElement("p");
    text.textContent = firstDiffText(branch) || "查看后决定是否采纳。";
    const tags = document.createElement("div");
    tags.className = "chip-row";
    tags.append(
      makeTag("基于版本 " + branch.baseVersion, "blue"),
      makeTag((branch.feedbackIds || []).length + " 条反馈", (branch.feedbackIds || []).length ? "amber" : ""),
      makeTag(branchStatusLabel(branch.status), branch.status === "proposed" ? "amber" : "green")
    );
    content.append(kicker, title, text, tags);
    const open = document.createElement("button");
    open.type = "button";
    open.className = "secondary-btn";
    setButtonContent(open, "open_in_new", "查看调整");
    open.addEventListener("click", function () {
      navigateTo("/plans/" + encodeURIComponent(planId) + "/branches/" + encodeURIComponent(branch.branchId));
    });
    card.append(content, open);
    return card;
  }

  function createBranchDetailSummary(branch, mainBranch) {
    const selected = branch.snapshot && branch.snapshot.selectedPlan || {};
    return createPageSummaryPanel({
      kicker: "PROPOSED CHANGE",
      title: selected.name || "待采纳调整",
      text: selected.branchNote || "这条调整仍在等待处理，采纳前不会覆盖当前方案。",
      tags: [
        { text: branchStatusLabel(branch.status), tone: branch.status === "proposed" ? "amber" : "green" },
        { text: "基于版本 " + branch.baseVersion, tone: "blue" },
        { text: (branch.feedbackIds || []).length + " 条反馈", tone: (branch.feedbackIds || []).length ? "amber" : "" },
      ],
      metrics: [
        { label: "当前版本", value: mainBranch ? String(mainBranch.baseVersion) : "暂无", tone: "blue" },
        { label: "调整版本", value: String(selected.version || branch.baseVersion + 1), tone: "amber" },
        { label: "状态", value: branchStatusLabel(branch.status), tone: branch.status === "proposed" ? "amber" : "green" },
      ],
    });
  }

  function createBranchDiffPanel(branch) {
    const panel = document.createElement("section");
    panel.className = "branch-diff-panel";
    const heading = document.createElement("div");
    heading.className = "section-heading";
    const titleWrap = document.createElement("div");
    const kicker = document.createElement("p");
    kicker.className = "section-kicker";
    kicker.textContent = "FEEDBACK IMPACT";
    const title = document.createElement("h2");
    title.textContent = "这次会调整什么";
    titleWrap.append(kicker, title);
    heading.appendChild(titleWrap);
    panel.appendChild(heading);
    const list = document.createElement("div");
    list.className = "branch-diff-list";
    (branch.diffSummary || []).forEach(function (item) {
      const row = document.createElement("article");
      row.className = "branch-diff-row";
      const icon = document.createElement("span");
      icon.className = "material-symbols-rounded";
      icon.setAttribute("aria-hidden", "true");
      icon.textContent = item.reaction === "concern" || item.reaction === "dislike" ? "warning" : "tune";
      const content = document.createElement("div");
      const rowTitle = document.createElement("h3");
      rowTitle.textContent = feedbackTargetLabel(item.targetType);
      const text = document.createElement("p");
      text.textContent = item.summary || "吸收这条反馈。";
      content.append(rowTitle, text);
      row.append(icon, content, makeTag(feedbackReactionLabel(item.reaction), item.reaction === "concern" || item.reaction === "dislike" ? "red" : "green"));
      list.appendChild(row);
    });
  ...
  function createBranchComparePanel(mainBranch, branch) {
    const panel = document.createElement("section");
    panel.className = "branch-compare-panel";
    const current = document.createElement("article");
    current.className = "branch-compare-card";
    const currentTitle = document.createElement("h2");
    currentTitle.textContent = "当前方案";
    const currentText = document.createElement("p");
    currentText.textContent = mainBranch && mainBranch.snapshot && mainBranch.snapshot.selectedPlan
      ? [mainBranch.snapshot.selectedPlan.name, "版本 " + mainBranch.baseVersion].filter(Boolean).join(" · ")
      : "当前还没有可对比的稳定版本。";
    current.append(currentTitle, currentText);
    const next = document.createElement("article");
    next.className = "branch-compare-card highlighted";
    const nextTitle = document.createElement("h2");
    nextTitle.textContent = "待采纳调整";
    const nextText = document.createElement("p");
    const selected = branch.snapshot && branch.snapshot.selectedPlan || {};
    nextText.textContent = [selected.name, firstDiffText(branch)].filter(Boolean).join(" · ");
    next.append(nextTitle, nextText);
    panel.append(current, next);
    return panel;
  }

  async function adoptPlanBranch(planId, branch) {
    setRouteNotice("正在采纳这条调整...", "");
    try {
      const response = await fetch("/api/plans/" + encodeURIComponent(planId) + "/branches/" + encodeURIComponent(branch.branchId) + "/adopt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          expectedVersion: branch.baseVersion,
          actor: "owner",
        }),
      });
      const data = await response.json();
      if (!response.ok || !data.ok || !data.branch) {
        throw new Error(data && data.error ? data.error : "branch_adopt_failed");
      }
      persistAdoptedBranchSnapshot(data.branch);
      setRouteNotice("已采纳为当前方案。", "success");
    } catch (error) {
      setRouteNotice("采纳失败，当前方案没有被覆盖。", "warning");
    }
  }

  async function rejectPlanBranch(planId, branchId) {
    setRouteNotice("正在拒绝这条调整...", "");
    try {
      const response = await fetch("/api/plans/" + encodeURIComponent(planId) + "/branches/" + encodeURIComponent(branchId) + "/reject", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          actor: "owner",
      ...
      const data = await response.json();
      if (!response.ok || !data.ok || !data.branch) {
        throw new Error(data && data.error ? data.error : "branch_reject_failed");
      }
      renderPlanBranchDetailContent(planId, data.branch, data);
    } catch (error) {
      setRouteNotice("拒绝失败，请稍后重试。", "warning");
    }
  }

  async function rollbackPreviousMain(planId) {
    setRouteNotice("正在恢复上一版...", "");
    try {
      const response = await fetch("/api/plans/" + encodeURIComponent(planId) + "/branches/rollback-previous-main", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      if (!response.ok || !data.ok || !data.branch) {
        throw new Error(data && data.error ? data.error : "branch_rollback_failed");
      }
      persistAdoptedBranchSnapshot(data.branch);
      setRouteNotice("已恢复到上一版。", "success");
    } catch (error) {
      setRouteNotice("暂时无法恢复上一版。", "warning");
    }
  }

  function persistAdoptedBranchSnapshot(branch) {
    if (!savedPlans || !branch || !branch.snapshot || !branch.snapshot.selectedPlan) return;
    const selectedPlan = cloneValue(branch.snapshot.selectedPlan);
    selectedPlan.previousMainBranchId = branch.previousMainBranchId || null;
    const snapshot = {
      snapshotId: branch.branchId,
      lineageId: branch.lineageId,
      sessionId: branch.snapshot.sessionId || null,
      version: branch.baseVersion,
      selectedPlan: selectedPlan,
      candidateSummaries: cloneValue(branch.snapshot.candidateSummaries || []),
      savedAt: new Date().toISOString(),
      dirty: false,
    };
    savedPlans.storeSnapshot(window.localStorage, snapshot);
    const workspace = savedPlans.reopenSnapshot(snapshot, null);
    workspace.dirty = false;
    savedPlans.storeSnapshotWorkspace(window.localStorage, snapshot.snapshotId, workspace);
    state.detailWorkspace = workspace;
    navigateTo("/saved-plans/" + encodeURIComponent(snapshot.snapshotId));
  }
  ...
  function branchStatusCopy(branch) {
    if (!branch) return "";
    if (branch.status === "proposed") return "这条调整还没有采纳，当前方案不会被覆盖。";
    if (branch.status === "rejected") return "这条调整已拒绝，记录会保留。";
    if (branch.status === "adopted") return "这条调整已成为当前方案。";
    return "这条记录已进入历史版本。";
  }

  function firstDiffText(branch) {
    const first = branch && branch.diffSummary && branch.diffSummary[0];
    return first && first.summary || "";
  }

  function canRollback(mainBranch) {
    return Boolean(mainBranch && mainBranch.previousMainBranchId);
  }
  ...
  function renderPlanDetailPage() {
    ...
    branchButton.className = "secondary-btn";
    setButtonContent(branchButton, "difference", "待采纳调整");
    branchButton.addEventListener("click", function () {
      navigateTo("/plans/" + encodeURIComponent(workspace.selectedPlanId) + "/branches");
    });
    const rollbackButton = document.createElement("button");
    rollbackButton.type = "button";
    rollbackButton.className = "ghost-btn";
    setButtonContent(rollbackButton, "undo", "回到上一版");
    rollbackButton.disabled = !canRollbackCurrentWorkspace(workspace);
    rollbackButton.addEventListener("click", function () {
      rollbackCurrentWorkspace(plan ? plan.id : workspace.selectedPlanId);
    });
    els.routeActions.append(listButton, shareButton, branchButton, rollbackButton, executeButton, saveButton);
  ...
  function canRollbackCurrentWorkspace(workspace) {
    const selected = workspace && workspace.selectedPayload;
    return Boolean(selected && selected.previousMainBranchId);
  }

  async function rollbackCurrentWorkspace(planId) {
    setRouteNotice("正在恢复上一版...", "");
    try {
      const response = await fetch("/api/plans/" + encodeURIComponent(planId) + "/branches/rollback-previous-main", {
        method: "POST",
