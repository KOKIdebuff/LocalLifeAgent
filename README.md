...
仓库当前没有正式的 `CONTRIBUTING.md`。

提交改动前至少应：

1. 保持静态 Demo 可独立运行。
2. 明确区分 Mock、已实现能力和规划中能力。
3. 避免提交 `.env`、密钥、SQLite、审计日志或本地缓存。
4. 按单一目的拆分 Conventional Commits。
5. 运行与改动范围匹配的测试；涉及共享契约或核心链路时运行完整基线。
6. 在 Pull Request 中说明行为变化、验证结果、兼容性和剩余风险。

<!-- 请补充：贡献流程、分支策略、代码风格和 Pull Request 模板 -->

## 许可证

仓库当前未提供 `LICENSE` 文件。

<!-- 请补充：项目许可证。许可证明确前，不应假定代码或资源可自由复制、修改或分发。 -->

项目维护者已确认 `assets/` 中的字体和图片为项目自有或允许随项目再分发；更精确的来源和许可信息见 `assets/README.md`。
...
本项目使用 FastAPI、Pydantic、HTTPX、pytest 和可选 LangGraph 构建后端实验链路。

<!-- 请补充：项目作者、贡献者和第三方数据/素材来源 -->
