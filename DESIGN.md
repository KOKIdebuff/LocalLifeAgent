...
后续会引入 LangGraph，但只作为 Python 后端 V4 alpha 的轻量编排层，用来组织意图识别、校验、规划适配、反馈复盘、候选记忆和候选记忆决策等节点。

LangGraph 不替代 `agent-core.js`，不接管 Mock Tools、方案评分、动态重排、执行队列、高影响动作确认和记忆敏感信息规则。LangGraph trace 只保留在后端日志或审计记录中，不映射到前端 `agentLoopTrace`。当前仓库也尚未安装或启用 LangGraph，具体边界见 `LANGGRAPH_INTEGRATION.md`。
...
- V4 alpha 后端增强负责意图识别、反馈复盘、结构化记忆和后续 LangGraph 轻量编排。
