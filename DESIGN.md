...
### 8.1 是否引入 LangGraph 的当前结论

基于 1 个月开发周期、Web UI、现场随机输入、可接真实 LLM、执行动作仍需 Mock 的约束，推荐路线是 LangGraph 轻量编排 + 业务逻辑自研。

LangGraph 负责流程调度、状态流转、分支、重试、兜底和 trace；核心业务逻辑自己写清楚。每个节点只做一件事，工具调用、Mock API、真实 API、异常处理都封装成独立 Tool。
...
- 先保证 LangGraph 编排、Mock API、POI 数据、候选方案和执行队列端到端跑通。
- 再扩充 30+ 城市和每类 5+ POI。
