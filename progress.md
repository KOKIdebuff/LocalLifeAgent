...
## 2026-06-08 V5 Plan Branch 分支闭环垂直切片

### 已完成

- 新增轻量 Plan Branch 后端能力：`GET /api/plans/{planId}/branches`、`POST /api/plans/{planId}/branches`、`GET /api/plans/{planId}/branches/{branchId}`、`POST /api/plans/{planId}/branches/{branchId}/adopt`、`POST /api/plans/{planId}/branches/{branchId}/reject`、`POST /api/plans/{planId}/branches/rollback-previous-main`。
- 新增 SQLite 分支表和分支事件表，保存 `main / derived` 生命周期、反馈来源、差异摘要、前一个主版本和审计事件。
- 协同反馈页“生成待采纳调整”已接通真实本地 API，可从当前反馈创建待采纳调整并跳转分支详情。
- 新增 `/plans/:planId/branches` 和 `/plans/:planId/branches/:branchId` 前端路由，支持查看待采纳调整、差异摘要、采纳、拒绝和回滚。
- 方案详情页新增“待采纳调整”和“回到上一版”入口，采纳后会保存为本地稳定快照并跳转到保存详情。
- 前端和后端测试已同步更新，覆盖分支路由、分支生命周期和协同反馈入口。

### 验证

- `node --check app.js`：通过。
- `node --check saved-plans.js`：通过。
- `node saved-plans-tests.js`：通过。
- `node ui-shell-tests.js`：通过。
- `.\\.venv\\Scripts\\python.exe -m pytest test_collaboration_api.py`：通过。
- `.\\.venv\\Scripts\\python.exe -m pytest test_execution_api.py test_runtime_api.py`：通过。
- `python -m py_compile server.py collaboration\\__init__.py collaboration\\adapter.py collaboration\\core.py collaboration\\errors.py collaboration\\models.py collaboration\\repositories.py`：通过。

### 未完成或阻塞

- 当前会话没有可用的 in-app Browser 操作工具，因此仍未完成 `/plans/:planId/branches` 与 `/plans/:planId/branches/:branchId` 的真实视觉与交互 QA。
- 安全扫描脚本未在仓库中找到独立可执行入口，因此这次只能依赖代码阅读和单测回归。

### 当前边界

- 本切片不做真实外部协作者写入，不做复杂分支树，不做局部合并。
- 采纳后的新主版本仍然只保存到本地快照与本地路由，不接真实发布或公网同步。
